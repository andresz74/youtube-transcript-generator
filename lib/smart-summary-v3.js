    ...
    try {
      ...
      if (transcriptLength <= summaryConfig.directTranscriptChars) {
        ...
        const response = await callModel({
          modelUrl,
          model,
          payload: inlineTranscript ? { transcript, videoID } : { videoID },
        ...
        for (let i = 0; i < chunks.length; i += 1) {
          ...
          logger.info(
            'smart-summary-v3 chunk-complete',
            JSON.stringify({ requestId, chunkIndex: i + 1, chunkCount, durationMs: stageDurations[`chunk${i + 1}Ms`] }),
          );
        }
